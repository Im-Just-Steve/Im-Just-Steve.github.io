...
window.showView = function showView(id) {
  ...
  document
    ...
    .forEach((x) => x.classList.toggle("active", x.dataset.nav === id));
...
function bindNavigation() {
  document
    ...
    .forEach((b) => b.addEventListener("click", () => showView(b.dataset.nav)));
  document
    .querySelectorAll("[data-action='new-flight']")
    .forEach((b) => b.addEventListener("click", () => openDialog()));
}
...
function bindDialog() {
  ...
  $("#deleteFlight").onclick = async () => {
    ...
    if (id && confirm("Delete this flight?")) {
  ...
  $("#flightForm").addEventListener("submit", async (e) => {
    ...
    if (!depart || !arrival) {
      alert("Please enter valid times as HH:MM.");
      return;
    }
    const flight = {
      id,
      ...
      nightMinutes: num("nightMinutes"),
      vfrMinutes: num("vfrMinutes"),
      ifrMinutes: num("ifrMinutes"),
      ...
      takeoffsDay: num("takeoffsDay"),
      takeoffsNight: num("takeoffsNight"),
      landingsDay: num("landingsDay"),
      landingsNight: num("landingsNight"),
...
function setupCompactInputs() {
  const d = $("#date"),
    p = $("#departTime"),
    a = $("#arrivalTime");
  d?.addEventListener("input", () => formatDateInput(d));
  p?.addEventListener("input", () => formatTimeInput(p));
  a?.addEventListener("input", () => formatTimeInput(a));
}
...
function openDialog(f = null) {
  ...
  $("#instrumentMinutesActual").value =
    f?.instrumentMinutesActual ?? f?.instrumentMinutes ?? 0;
  ...
  $("#takeoffsDay").value =
    f?.takeoffsDay != null ? f.takeoffsDay : f ? oldTakeoffs : 1;
  $("#takeoffsNight").value = f?.takeoffsNight != null ? f.takeoffsNight : 0;
  $("#landingsDay").value =
    f?.landingsDay != null ? f.landingsDay : f ? oldLandings : 1;
...
function calculateDuration(depart, arrival) {
  ...
  const [dh, dm] = depart.split(":").map(Number),
    [ah, am] = arrival.split(":").map(Number);
  let start = dh * 60 + dm,
    end = ah * 60 + am;
...
function total(field) {
  return flights.reduce((s, f) => s + (Number(f[field]) || 0), 0);
}
/* Dashboard */
function renderStats() {
  const totalHours = sumRoundedHours(flights);
  const picFlights = flights.filter(
    (f) =>
      f.role === "P.1" || f.role === "P.1/S" || f.role === "P.1 (Instructor)",
  );
  const dualFlights = flights.filter((f) => f.role === "P.U/T");
  const instructorFlights = flights.filter(
    (f) => f.role === "P.1 (Instructor)",
  );
  ...
  const actualInstrumentHours = sumRoundedHours(
    flights,
    "instrumentMinutesActual",
  );
  const legacyInstrumentHours = flights.some(
    (f) => f.instrumentMinutesActual != null,
  )
    ? 0
    : sumRoundedHours(flights, "instrumentMinutes");
  const simulatedInstrumentHours = sumRoundedHours(
    flights,
    "instrumentMinutesSimulated",
  );
  const instrumentHours =
    actualInstrumentHours + legacyInstrumentHours + simulatedInstrumentHours;
  const instructorHours = sumRoundedHours(instructorFlights);
  const takeoffsDay = total("takeoffsDay"),
    takeoffsNight = total("takeoffsNight");
  const landingsDay = total("landingsDay"),
    landingsNight = total("landingsNight");
...
function attachFlightClicks() {
  document
    ...
    .forEach(
      (el) =>
        (el.onclick = () =>
          openDialog(flights.find((f) => f.id === el.dataset.id))),
    );
}
function renderRecent() {
  const data = flights.slice(0, 6);
  $("#recentFlights").innerHTML = data.length
    ? data.map(flightHTML).join("")
    : `<div class="empty">No flights yet. Tap “Log flight” to start.</div>`;
  attachFlightClicks();
}
function renderLogbook() {
  ...
  $("#allFlights").innerHTML = data.length
    ? data.map(flightHTML).join("")
    : `<div class="empty">No matching flights.</div>`;
  attachFlightClicks();
}
...
function getDayNightCounts(f) {
  ...
  return {
    takeoffsDay: tDay,
    takeoffsNight: tNight,
    takeoffsTotal: tDay + tNight,
    landingsDay: lDay,
    landingsNight: lNight,
    landingsTotal: lDay + lNight,
  };
}
function buildStatsRows(data, mode) {
  ...
  if (mode === "class") {
    ...
    data.forEach((f) => {
      const aircraft = String(f.aircraft || "")
        .trim()
        .toUpperCase();
...
function statsRowsTable(rows) {
  if (!rows.length)
    return '<div class="empty">No flights in this period.</div>';
  return `<div class="stats-table-wrap"><table class="stat-table stats-breakdown-table">
    ...
    <tbody>${rows
      .map(
        (
          r,
        ) => `<tr><td>${escapeHTML(r.name)}</td><td>${displayHours(r.hours)} h</td>
      <td>${r.takeoffsDay}</td><td>${r.takeoffsNight}</td><td>${r.takeoffsTotal}</td>
      <td>${r.landingsDay}</td><td>${r.landingsNight}</td><td>${r.landingsTotal}</td></tr>`,
      )
      .join("")}</tbody>
  </table></div>`;
}
function statsGroups(data, mode) {
  ...
  if (mode === "class")
    aircraftClasses.forEach((c) =>
      (c.types || []).forEach((t) =>
        typeToClass.set(String(t).trim().toUpperCase(), c.name),
      ),
    );
  data.forEach((f) => {
    const type = String(f.aircraft || "")
      .trim()
      .toUpperCase();
    ...
    const name =
      mode === "class"
        ? typeToClass.get(type) || "Unclassified"
        : mode === "registration"
    ...
    .map(([name, items]) =>
      items.reduce(
        (a, f) => {
          ...
          a.hours += hours;
          if (String(f.flightType || "").toUpperCase() === "IFR")
            a.ifrHours += hours;
...
function renderStatistics() {
  const active =
    $("#statistics .filter-btn.active")?.dataset.statsRange || "all";
  let data = flights;
  if (active === "90") {
    const end = new Date();
    ...
    const from = $("#statsFrom").value,
      to = $("#statsTo").value;
    data =
      from && to ? flights.filter((f) => f.date >= from && f.date <= to) : [];
  }
  ...
  if (dayNightFilter === "day")
    data = data.filter((f) => {
      const n = getDayNightCounts(f);
      return n.takeoffsDay > 0 || n.landingsDay > 0;
    });
  if (dayNightFilter === "night")
    data = data.filter((f) => {
      const n = getDayNightCounts(f);
      return n.takeoffsNight > 0 || n.landingsNight > 0;
    });
  ...
  if (
    mode !== "all" &&
    (!statsSelectedItem || !groups.some((g) => g.name === statsSelectedItem))
  )
    statsSelectedItem = groups[0]?.name || null;
    ...
    if (mode === "aircraft")
      selectedData = data.filter(
        (f) =>
          String(f.aircraft || "")
            .trim()
            .toUpperCase() === String(statsSelectedItem).toUpperCase(),
      );
    else if (mode === "registration")
      selectedData = data.filter(
        (f) =>
          String(f.registration || "")
            .trim()
            .toUpperCase() === String(statsSelectedItem).toUpperCase(),
      );
    else if (mode === "class") {
      ...
      const types = new Set(
        (cls?.types || []).map((t) => String(t).trim().toUpperCase()),
      );
  ...
  const aggregate = (list) =>
    list.reduce(
      (a, f) => {
        const n = getDayNightCounts(f);
        const hours = entryHours(Number(f.blockMinutes) || 0);
        const actual = entryHours(
          Number(f.instrumentMinutesActual ?? f.instrumentMinutes) || 0,
        );
        ...
        if (rules === "VFR") {
          ...
          const mixedTotal =
            (Number(f.vfrMinutes) || 0) + (Number(f.ifrMinutes) || 0);
          if (mixedTotal > 0) {
            a.nightVfr += entryHours(
              (nightMinutes * (Number(f.vfrMinutes) || 0)) / mixedTotal,
            );
            a.nightIfr += entryHours(
              (nightMinutes * (Number(f.ifrMinutes) || 0)) / mixedTotal,
            );
          }
        }
        return a;
  ...
  const instructor = aggregate(
    selectedData.filter((f) => f.role === "P.1 (Instructor)"),
  );
  const night = aggregate(
    selectedData.filter((f) => (Number(f.nightMinutes) || 0) > 0),
  );
  const title =
    mode === "all"
      ? "All Flights"
          ...
          mode === "all"
            ...
            : groups.length
              ? groups
                  .map(
                    (g) =>
                      `<button type="button" class="filter-btn stats-item-btn ${g.name === statsSelectedItem ? "active" : ""}" data-stats-item="${escapeHTML(g.name)}">${escapeHTML(g.name)}</button>`,
                  )
                  .join("")
...
async function exportData() {
  ...
  a.download = `skylog-backup-${new Date().toISOString().slice(0, 10)}.json`;
  a.click();
...
async function importData(e) {
  ...
  try {
    ...
    if (!valid.length)
      throw new Error("The backup contains no valid flight records.");
    ...
    alert(
      `Could not import this SkyLog backup. ${err.message || "The file is invalid or unsupported."}`,
    );
  } finally {
    e.target.value = "";
  }
...
async function clearData() {
  if (confirm("Delete every flight from this device? This cannot be undone.")) {
    await clearFlights();
    await refresh();
